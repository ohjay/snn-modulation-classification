...
def get_binary_frame(evs, size=(346, 260), ds=1):
    tr = sparse_matrix(
        (2*evs[:, 3]-1, (evs[:, 1]//ds, evs[:, 2]//ds)), dtype=np.int8, shape=size)
    return tr.toarray()
...
def chunk_evs(evs, deltat=1000, chunk_size=500, size=[304, 240], ds=1):
    ...
    for i, t in enumerate(ts):
        ...
        if idx_end > idx_start:
            get_binary_frame_np(
                chunks[i, ...], evs[idx_start:idx_end], size=size, ds=ds)
        idx_start = idx_end
    return chunks
...
if __name__ == "__main__":
    import h5py
    dataset = h5py.File(
        '/home/eneftci_local/Projects/share/data/massiset/massiset_sparse.hdf5', 'r')
    evs = dataset.get('backpack')['data_train'].value
    cevs = chunk_evs(evs, chunk_size=500, deltat=1000,
                     ds=4, size=[304//4, 240//4])
