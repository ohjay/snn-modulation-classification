...
def parse_args():
    parser = argparse.ArgumentParser(description='DCLL for RadioML')
    parser.add_argument('--batch_size', type=int, default=64, metavar='N', help='input batch size for training')
    parser.add_argument('--batch_size_test', type=int, default=64, metavar='N', help='input batch size for testing')
    ...
    parser.add_argument('--output', type=str, default='Results_RadioML/', help='folder name for the results')
...
class ReferenceConvNetwork(torch.nn.Module):
    def __init__(self, im_dims, convs, loss, opt, opt_param):
        ...
        self.linear = torch.nn.Linear(np.prod(latent_size()), 24).to(device)
...
if __name__ == '__main__':
    ...
    log_dir = os.path.join('runs/', 'pytorch_conv3L_RadioML_', current_time + '_' + socket.gethostname() +'_' + args.comment, )
    print(log_dir)
    ...
    writer = SummaryWriter(log_dir = log_dir, comment='RadioML Conv')
    ...
    from data.load_radio_ml import get_radio_ml_loader
    from data.utils import image2spiketrain
    train_data = get_radio_ml_loader(args.batch_size, train=True, taskid=0)
    gen_train = iter(train_data)
    gen_test = iter(get_radio_ml_loader(args.batch_size_test, train=False, taskid=1))
